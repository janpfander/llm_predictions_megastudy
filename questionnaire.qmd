---
title: "Questionnaire: 16 information interventions to strengthen trust in climate scientists in the US"
...
::: callout-note
This is the instrument for the **Silicon Sample Benchmark** — the non-interactive subset
of the parent [megastudy](https://janpfander.github.io/trust_climate_scientists/). It is
generated from the megastudy questionnaire, restricted to the 16 text-based interventions
predicted in this benchmark (the four interactive chatbot and value-similarity arms are
excluded). Intervention texts are read from the shared interventions file.
:::
...
interventions <- read_csv(here::here("data/interventions.csv")) |>
  mutate(
    ...
    # `title` is the analysis label (cleaning.qmd maps code_name -> title);
    # some titles carry a trailing newline that renders badly in headings
    title = str_trim(title)
  ) |>
  # benchmark scope: drop the four interactive arms (3 LLM-chatbot + value similarity)
...
for (i in 1:nrow(interventions)) {
  cat(glue(
    "
### {i}: {interventions$title[i]}

#### Authors: {interventions$names[i]}

**Code name**: {interventions$code_name[i]}

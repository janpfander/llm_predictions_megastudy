...
12. **Subgroup distributions.** Averaging outcome distributions over demographic groups can hide an approach that serves most groups well but consistently misses one. We report the gap between the worst- and best-served group in terms of distribution similarity [@parkLLMAgentsGrounded2026], with under-dispersion as the headline distribution diagnostic, since too-similar synthetic responses are the problem the literature flags most often. Groups under 30 respondents are skipped and named, with their sizes, wherever they are reported. See [*Section 1 — Average Treatment Effects*](#sec-section-1) (response distributions), [*Section 2 — Subgroup Effects*](#sec-section-2), and [*Section 3 — Demographic Baseline Calibration*](#sec-section-3).
...
`compare_demographic_baselines()` computes, for each moderator, the RMSE between human and synthetic sample cell means in the control condition. (The function also returns a per-moderator Pearson correlation, which the benchmark does not report — see [*Section 3 — Demographic Baseline Calibration*](#sec-section-3) for why.) `compare_demographic_predictability()` fits separate OLS regressions of the outcome on each moderator plus condition fixed effects, returning $R^2$ and dummy-coded coefficients for humans and for the submission. `demographic_parity_gap()` quantifies, for each moderator, the gap between the worst- and best-served demographic group in the control condition. For example, if an approach's control-condition baseline is off by 6 pp for Republicans but only 1 pp for Democrats, the parity gap for partisan identity is 5 pp, with Republicans the worst-served group. Groups under 30 respondents on either side are skipped, and the skipped groups are named, with their sizes, wherever the parity gap is reported (see [*Section 3 — Demographic Baseline Calibration*](#sec-section-3)).
...
The cross-team analysis (see [*Presentation of findings*](#sec-findings), below) reuses the comparison functions specified above and adds the scoring helpers below (in `R/functions/statistics.R`), each shown in full and explained before it.
...
Each cross-team score is a single pooled number, one per approach per metric (see [*Presentation of findings*](#sec-findings), below). A bare point invites over-reading. If one approach scores Pearson *r* = 0.60 and another 0.55, is the first genuinely better, or is the gap an artifact of the particular interventions that happened to be tested? The interval answers that question. It guards against ranking approaches on differences that would not survive a different, equally reasonable test.
...
### Section 3 — Demographic Baseline Calibration {#sec-section-3}
